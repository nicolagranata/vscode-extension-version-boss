...
function fv_row_fill(filler_label, filler_value, row_to_fill, row_fill_char, label_value_separator_char, max_label_len, max_value_len, frame_char, left_right_frame_char, space_char, sections_header_separator_chars, bool_leave_open_row, eol) {
	...
	if (left_right_frame_char != "") {
		...
		copyrights_header_separator = left_right_frame_char + spacer + copyrights_header_separator + spacer + left_right_frame_char.reverse();
	} else {
...
function fv_versioning(registered_command, root_item, semver_boss_command, step_value, bool_autosave) {
	...
	typeof (step_value) == "undefined" || step_value == 0 || isNaN(Number(step_value)) || step_value == "" ? step_value = 1 : vz;
	...
	if (semver_boss_command == "getsettingshelp") {
		...
		if (boundary_close_char == boundary_open_char) {
			boundary_open_char == "/" ? boundary_close_char = "!" : boundary_close_char = "/";
		}
		...
		var comments_block_close_default = comments_block_open_default.reverse();
		...
		switch (filename_ext) {
			...
			case "py":
				comments_block_open = "'''";
				comments_block_close = comments_block_open.reverse();
				comments_row_open = "#";
				comments_row_close = comments_row_close_default;
				break;
			case "htm":
		...
		switch (semver_boss_command) {
			case "chanver":
				...
				case "patch":
					...
					new_ver_ra[2] = new_ver_ra[2] * 1 + step_value;
					// per step down
		...
		if (semver_boss_command == "copyrights") {
			...
			copyrights_top_bottom_row = copyrights_top_bottom_row_half + copyrights_top_bottom_row_half.reverse();
			...
			copyrights_fill_row = copyrights_left_and_right_frame_char + copyrights_fill_row.substr(0, copyrights_fill_row.length - 2 * copyrights_left_and_right_frame_char.length) + copyrights_left_and_right_frame_char.reverse();
