	...
	if (semver_boss_command == "getsettingshelp") {
		...
		for (i in settings_obj) {
			...
			if (copyrights_item_label.indexOf(root_item + ".copyrights.") >= 0) {
				...
				copyrights_item_label = camelToLabel(copyrights_item_label);
		...
		var leading_chars_allowed_on_pattern_match_str = String(settings_obj[ root_item + ".settings.leadingCharsAllowedInPatternMatch" ]);
		var leading_chars_allowed_on_pattern_match_str = leading_chars_allowed_on_pattern_match_str.replace(new RegExp("([\t ]+)", "gim"), "");
		var leading_chars_allowed_on_pattern_match_ra = leading_chars_allowed_on_pattern_match_str.split("");
		var leading_chars_allowed_on_pattern_match_re = "\\" + leading_chars_allowed_on_pattern_match_ra.join("|\\");
		var re_semver_extended_enclosers = "(\"|\')(" + leading_chars_allowed_on_pattern_match_re + "|)";
		...
		if (semver_boss_command == "getinfosarray") {
			...
			if (editor_contents_str.match(new RegExp(variable_name_regex_escaped + "([\t ]*)\=", "gim")) == null || !check_if_vnfc_exists) {
				...
				done_msg.push([ "warning", "Copyrights Infos Array \"" + variable_name + "\" seems to be already present in your code. Please delete them or check conflicts in your code with variable name \"" + variable_name + "\" in \"Settings / " + package_obj.displayName + " / " + camelToLabel(variable_name_id) + "\" to be able to update them." ]);
			}
		}
		...
		if (semver_boss_command != "copyrights" && semver_boss_command != "licenses" && semver_boss_command != "getinfosarray") {
			...
			if (semver_extended_match_ra != null) {
				...
				editor_contents_str = editor_contents_str.replace(new RegExp(re_semver_extended_full, regexp_mode), "$1$2" + new_ver + "$8");
			}
		}
